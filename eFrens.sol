...
Henloooo Frensss 
welcome to our cozzy and frenly proyect!!

Join our telegram
 https://t.me/evmosfrens

Check our web

https://www.aifrens.art

 */
/**
 * 
 * The eFrens contract itself has been heavily annotated to allow all frens
...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have a EVMOS balance of at least `value`.
...
contract eFrens is Context, IERC20 {
    ...
    string public  _name = "evmos Frens";
    
    string public  _symbol = "eFRENS";
    ...
    address public usd = 0x51e44FfaD5C2B122C8b635671FCC8139dc636E82; //Usdc
...
constructor (){
        ...
        // Sets wallets.
        growthAddress = 0x731AD54D3cF7867994432a773ef13e5fe8EBAbB9;
        funAddress = 0x731AD54D3cF7867994432a773ef13e5fe8EBAbB9;
        charityAddress = 0x731AD54D3cF7867994432a773ef13e5fe8EBAbB9;
        ...
        // Sets router address
        DEXRouterAddress = 0xFCd2Ce20ef8ed3D43Ab4f8C2dA13bbF1C6d9512F;
     ...
     *  (but not buy or receive) eFrens even if their balance exceeds the max wallet limit.
     ...
     *  eFrens from a pool if the withdrawal would violate the max Tx or max wallet limits.
     ...
     */
    function addLiquidity(uint256 tokenAmount, uint256 EVMOSAmount) private {
        approveInternal(address(this), address(DEXRouter), tokenAmount);        
        DEXRouter.addLiquidityETH{value: EVMOSAmount}(address(this),tokenAmount, 0, 0, address(this), block.timestamp);     
    }
    
    /** Swaps tokens for EVMOS having grabbed the router address and approved both tokens. */
    function swapTokensForEVMOS(uint256 tokenAmount) private {
     ...
     * It sells half of the eFrens queued for LP conversion when the threshold is reached, triggered by sells.
     * It then grabs the amount of EVMOS the swap creates.
     * Finally it combines the EVMOS with eFrens to add liquidity to the main WEVMOS-eFrens pool to the DEX.
     * Over time, it will cause a build-up of eFrens in this contract address.
     * This happens because swapping half the eFrens into EVMOS reduces the price a bit.
     * In order to mitigate any centralization risk, eFrens built up in the contract will be recovered and locked
    ...
    /** Returns the total eFrens supply in uint256 format - 1000000000000000 i.e. 1,000,000 tokens. */
    ...
    /** Returns the token ticker - eFrens. */
    function symbol() external view override returns (string memory) {
        return _symbol;   
    }

    /** Returns the token name - eFrens. */
    function name() external view override returns (string memory) {
        return _name;   
    }

    /** Returns the eFrens balance of the address queried in uint256 format e.g. 5000000000000 = 5,000 tokens. */
    ...
    // To receive EVMOS from the router when swapping.
     ...
     *  EVMOSnos chain is new at the time this contract is deployed, and there will be no established
     *  DEX as a result.
     * 
     ...
     *  it could be called to change which pair autoEVMOS liquidity is added to.
     * 
    ...
    function ExtendLiquidityLock() external onlyOwner() {
     ...
     * bound is higher at 5%. eFrens can be deflationary or not - frens decide.
     * 
    ...
    function setFunAddress(address _newaddress) external onlyOwner() {
    ...
    /**
     * Function: Allows the owner to toggle autoEVMOS liquidity on and off.
     *  
     ...
     *  that may arise, such as being unable to withdraw eFrens from a token
     ...
     *  which would result in a significant build-up of eFrens in this contract address
     *  due to the tax and reflections gained. The eFrens could then be recovered from
     ...
     *  because they hold most of the eFrens. The burn address is also excluded for The
     ...
     *  deemed to threaten the future of the project by holding more eFrens than intended,
     ...
     *  
     * Justification: This function exists primarily to whitelist eFrens pools so that 
     ...
     *  issues: a fren may be unable to unstake eFrens from a pool, harvest from a farm or 
     *  break LP if the Tx would take them over the max wallet limit; a fren may be unable 
     *  to receive eFrens from giveaways and competitions. These isues are only expected to
     ...
     *  to unstake eFrens or break LP due to it violating the max Tx limit. It can also be used
     ...
     *  from the eFrens contract.
     *  
     * Justification: As above but for any token. The owner does not preclude the possibility of 
     *  taking some of this LP, breaking it, distributing the eFrens back to the community via 
     *  the Growth wallet and keeping some EVMOS for personal use. This is hoped to be viewed as a 
     *  reasonable action due to the fact that 0% of the initial supply is held in reserve as an ROI. 
     * 
     * Risk statement: As above but for any token. Could be called to withdraw eFrens awaiting
    ...
    /**
     * Function: Allows the owner to withdraw all EVMOS from the eFrens contract.
     *  
     * Justification: The reason this is called a recover function is that its intended
     *  use is to recover EVMOS sent accidentally to the eFrens contract address. This is
     *  a common and often costly mistake that usually results in tokens being lost forever.
     *  Some EVMOS will build up in the contract over time as a byproduct of the autoLiquidity 
     *  function. This is because every time half the amount of the auto-liquidity threshold 
     *  in eFrens is swapped for EVMOS and combined to form LP, the swap itself marginally reduces 
     *  the price of eFrens. The autoLiquidity function does not account for this the next 
     *  time it is called.
     * 
     * Risk statement: The amount of EVMOS that builds up due to auto-liquidity is likely to
     *  become significant over time, and this presents a centralization risk. 
     * 
     * Mitigation: The EVMOS will most likely be locked or redistributed to the community, with a 
     *  reasonable amount that could be kept aside for the owner's personal use after consultation 
     *  with the TrueDao.
     */
    function recoverEVMOS() external onlyOwner()  { 
        emit EVMOSRecovered(payableAddr(), balanceOf(address(this)));
        payableAddr().transfer(address(this).balance);
    }
    
    /** Gets the address to which tokens can be recovered from the eFrens contract - the owner wallet. */
